...
{
    class SuperAttacco : IAttacco
    {
        
        string risultato;
        public string Tipodiattacco() => "SuperAttacco";
        public string Attacco(Player p1, Player p2)
        {
            int DannoTotale;
            if (p1.GetAttaccoPlayer() > p2.GetDifesaPlayer() && p1.Vita > 0 && p2.Vita > 0)
            {
                DannoTotale = (p1.GetAttaccoPlayer() - p2.GetDifesaPlayer()) * 2;
                p2.Vita -= DannoTotale;
                if (p2.Vita <= 0)
                {
                    p1.Esperienza += 10;
            ...
            {
                DannoTotale = (p2.GetDifesaPlayer() - p1.GetAttaccoPlayer()) * 2;
                p1.Vita -= DannoTotale;
                if (p1.Vita <= 0)
                {
                    p2.Esperienza += 10;
